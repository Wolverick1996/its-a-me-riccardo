...
A practical gotcha worth watching for: a gradient-heavy area can look flat from a couple of representative-looking sample points; reading one pixel in the middle of a boundary or divider and assuming it holds constant across the whole region is easy to get away with on a genuinely flat area and easy to get badly wrong on a gradient one. Scan the entire relevant row/column/grid and look at the actual numbers, reserving single-point sampling for regions already confirmed flat by checking more than one point.
